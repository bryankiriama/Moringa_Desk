...
from backend.app.repositories.follow_repo import create_follow
...
def test_followers_notified_on_new_answer() -> None:
    ...
    follower_token = _register_and_get_token(client, "follower@example.com")

    owner_id = _decode_sub(owner_token)
    follower_id = _decode_sub(follower_token)
    ...
    with session_module.SessionLocal() as session:
        ...
        create_follow(session, user_id=follower_id, question_id=question.id)
    ...
    with session_module.SessionLocal() as session:
        owner_notifications = list_notifications(session, user_id=owner_id)
        follower_notifications = list_notifications(session, user_id=follower_id)

        assert len(owner_notifications) == 1
        assert owner_notifications[0].type == "answer_posted"

        assert len(follower_notifications) == 1
        assert follower_notifications[0].type == "followed_question_answer"


def test_no_follow_notification_for_actor_or_owner() -> None:
    client, session_module = setup_app_with_sqlite()
    ...
    with session_module.SessionLocal() as session:
        ...
        create_follow(session, user_id=owner_id, question_id=question.id)
    ...
    with session_module.SessionLocal() as session:
        ...
        # should be no followed_question_answer, and no answer_posted for self-answer
        assert len(items) == 0
