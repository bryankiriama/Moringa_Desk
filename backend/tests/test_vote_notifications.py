...
def test_vote_on_question_creates_notification_for_owner() -> None:
    ...
    with session_module.SessionLocal() as session:
        ...
        assert len(items) == 1
        assert items[0].type == "vote_received"
        assert items[0].payload.get("value") == 1


def test_vote_on_answer_creates_notification_with_question_id() -> None:
    client, session_module = setup_app_with_sqlite()
    ...
    with session_module.SessionLocal() as session:
        ...
        assert len(items) == 1
        assert items[0].type == "vote_received"
        assert items[0].payload.get("question_id") == str(question.id)
        assert items[0].payload.get("value") == 1


def test_no_notification_for_self_vote() -> None:
    client, session_module = setup_app_with_sqlite()
